...
def main():
    ...
    parser.add_argument(
        "--additional-special-symbols", type=str, nargs="+",
        help="Tags that should be included in the vocabulary.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Number of lines to process in one batch.")
    args = parser.parse_args()

    if args.additional_special_symbols is not None and not args.marian_yaml:
        print(
            f"Special tokens can only be included in Maria YAML.",
            file=sys.stderr)
    ...
    for input_file in args.input:
        ...
        for line in input_file:
            ...
            if len(current_buffer) > args.buffer_size:
    ...
    if args.marian_yaml:
        ...
        if args.additional_special_symbols is not None:
            for sym in args.additional_special_symbols:
                yaml_vocabulary[sym] = len(yaml_vocabulary)
